(function () {
  ...
  const POLL_MS = 15000; // 배포 환경에서는 15초 간격 (호출량 절약)
    ...
    return '<section class="panel">' +
      '<div class="panel-head"><h2>관심종목 <span class="count num">' + quotes.length + "</span></h2>" +
        '<div class="head-actions">' +
          '<span class="hint">' + POLL_MS / 1000 + "초마다 갱신</span>" +
  ...
  function render() {
    app.innerHTML =
      ...
      '<footer class="note">가상의 예수금 ₩10,000,000으로 시작하는 연습용 모의투자입니다 · 실제 매매·투자 판단의 근거로 사용하지 마세요<br>' +
        "잔고와 거래내역은 이 브라우저에만 저장되며, 다른 사람에게는 보이지 않습니다</footer>" +
  ...
  async function boot() {
    ...
    await refreshAll();
    setInterval(async () => {
      if (addModal.open) return; // 입력 중에는 다시 그리지 않음
      await refreshQuotes();
      recomputePortfolio();
      render();
    }, POLL_MS);
